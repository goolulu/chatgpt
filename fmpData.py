...
def get_financial_score(symbol: str):
    url = f'https://financialmodelingprep.com/api/v4/score?symbol={symbol}&{apikey}'
    return json.dumps(requests.get(url).json())


@calculate_execution_time
def get_advanced_dcf_valuation(ticker):
    url = f'https://financialmodelingprep.com/api/v4/advanced_discounted_cash_flow?symbol={ticker}&{apikey}'
    return json.dumps(requests.get(url).json())


available_functions = {
    "get_income_statement": fmpsdk.income_statement,
    "get_balance_sheet": fmpsdk.balance_sheet_statement,
    ...
    # "get_advanced_dcf_valuation": fmpsdk.cash,
...
if __name__ == '__main__':
    function_to_call = available_functions.get("get_dcf_valuation")
    print(function_to_call(apikey, 'AAPL'))
    print(get_dcf_valuation('AAPL'))
